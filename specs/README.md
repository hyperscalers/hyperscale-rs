...
| **D** | [straddler_settlement.qnt](straddler_settlement.qnt) | Straddler settlement layered on C: the settled-set fence over a shard scheduled to terminate, sweeps, late-EC materialization. Healthy instance holds atomicity; twins remove the scheduled-terminal layer (`straddler_preboundary`) and the fence entirely (`straddler_unfenced`, H2). Cross-validated against the implementation by the `split_straddler_asymmetric_ec_partition` sim test | INV-RESHAPE-5/6 | Verified |
