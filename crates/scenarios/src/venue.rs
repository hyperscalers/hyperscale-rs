...
/// a leg that already moved value. The venue declines, and its refusal
/// receipt writes `Never` at the crossing's decline key; the caller
/// reclaims off that `Never`, crediting the vault from the record cell
/// the leg wrote. What the caller is out afterwards is the price and
/// nothing else.
///
