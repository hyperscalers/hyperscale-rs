...
use hyperscale_types::{BlockHash, VerifiedQuorumCertificate};

/// Single-slot stash for a QC awaiting its block header.
#[derive(Default)]
pub struct DeferredQc(Option<(BlockHash, VerifiedQuorumCertificate)>);
...
impl DeferredQc {
    ...
    pub fn defer(&mut self, block_hash: BlockHash, qc: VerifiedQuorumCertificate) {
    ...
    pub fn take_for(&mut self, block_hash: BlockHash) -> Option<VerifiedQuorumCertificate> {
