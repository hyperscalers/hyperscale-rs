...
use hyperscale_types::{BlockHash, QuorumCertificate};

/// Single-slot stash for a QC awaiting its block header.
#[derive(Default)]
pub struct DeferredQc(Option<(BlockHash, QuorumCertificate)>);
...
impl DeferredQc {
    ...
    pub fn defer(&mut self, block_hash: BlockHash, qc: QuorumCertificate) {
    ...
    pub fn take_for(&mut self, block_hash: BlockHash) -> Option<QuorumCertificate> {
