...
impl BftState {
    ...
    fn try_vote_on_block(
        ...
        if let Some(pending) = self.pending_blocks.get(&block_hash) {
            if let Some(block) = pending.block() {
                ...
                // Verify per-target provision tx-roots if block has any.
                if self
                    .verification
                    .needs_provision_tx_root_verification(&block)
                {
                    verification_actions.extend(
                        self.verification
                            .initiate_provision_tx_root_verification(block_hash, &block, topology),
                    );
                }
    ...
    pub fn on_block_root_verified(
        ...
        let pipeline_ok = match kind {
            ...
            VerificationKind::ProvisionTxRoots => self
                .verification
                .on_provision_tx_roots_verified(block_hash, valid),
        };
    ...
    fn commit_block_and_buffered(
        ...
        loop {
            ...
            // itself on `is_complete()` (all txs + certs + provisions
            // received), and `construct_block` attaches the provisions
            // inline on `Block::Live` — no external cache lookup needed.
