...
/// block validity: it only quiesces the local node. One fence serves the
/// node, shared by handle: the consuming coordinators read it, and only
/// the node engages and clears it, so the consumers cannot disagree about
/// which shards are fenced. Its lifecycle:
///
...
pub struct ForkFence {
    fences: Arc<RwLock<BTreeMap<ShardId, Fence>>>,
}
...
impl ForkFence {
    ...
    fn read(&self) -> RwLockReadGuard<'_, BTreeMap<ShardId, Fence>> {
        self.fences.read().unwrap_or_else(PoisonError::into_inner)
    }

    fn write(&self) -> RwLockWriteGuard<'_, BTreeMap<ShardId, Fence>> {
        self.fences.write().unwrap_or_else(PoisonError::into_inner)
    }
    ...
    #[must_use]
    pub fn engaged(&self) -> Vec<(ShardId, BlockHeight)> {
        self.read()
            .iter()
...
mod tests {
    ...
    /// One fence serves the node: every handle reads what any of them
    /// engaged, and a clear through one lifts it for all.
    #[test]
    fn every_handle_reads_the_one_fence() {
        let shard = ShardId::leaf(1, 0);
