...
The rule with teeth: **state machines never read a clock; they are told the time** (INV-DET-2). In production, runners stamp `LocalTimestamp` from the system clock at the event boundary; in simulation, from the event queue. Any `Instant::now()` inside an FSM is a determinism bug by definition — one such (in the shared sync FSM) caused a genuinely load-dependent test flake, which is exactly the class of defect the rule exists to make structurally impossible.
...
- **It is the bridge to formal verification.** Because every protocol component is already a pure transition function with explicit inputs — not code entangled with sockets, threads, and clocks — extracting a model is a translation task, not an archaeology task, and any modeled property can be cross-validated against the running system through the scenario layer. The invariant register ([08-invariants.md](08-invariants.md)) is written against these transition functions.

## 6. Properties (summary)

- **INV-DET-1 (purity).** Every protocol transition is a pure function of `(state, now, event)`; all effects are explicit `Action` values.
- **INV-DET-2 (fed time).** No protocol component reads a clock or OS randomness; time is an input, consensus randomness is beacon-attested, harness randomness is seeded.
- **INV-DET-3 (harness parity).** Identical protocol code runs under simulation and production; behavioral scenarios are harness-portable, and cross-harness divergence always indicates a real defect.
- **INV-DET-4 (seeded reproducibility).** A simulation run — fault injection included — is byte-identical for a given seed.
- **INV-DET-5 (deterministic encoding).** All hashed/signed/wire types encode deterministically (SBOR, ordered collections, bounded lengths).
- **INV-DET-6 (typestate verification).** Verified status is a type-level marker: unobtainable from wire bytes, preserved only across in-process handoffs within one trust domain (one process = one operator). Nothing is re-verified locally; nothing unverified masquerades as verified.
