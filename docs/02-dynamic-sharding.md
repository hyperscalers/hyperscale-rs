...
**The fence** (a consensus rule, enforced at vote and at local finalization): a block carrying a `FinalizedWave` whose certificate names past-terminal shard `P` is votable iff the wave id is in `S_P`. Not yet acquired → defer the vote; acquired and absent → reject; past the retention horizon → reject absolutely. Symmetric checks gate a replica's own finalization path, so an honest node never produces what the fence would refuse (INV-RESHAPE-5).
