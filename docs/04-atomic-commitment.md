...
- Proposal selection is deterministic (hash-ordered iteration up to the block budget — a transaction count today, gas planned), so all replicas agree on eligibility reasoning.
...
Validators vote on the wave's `global_receipt_root`; 2f+1 matching votes form the shard's **ExecutionCertificate** with the explicit per-transaction outcome vector. A wave finalizes only when ECs from **all** participating shards agree — one local, plus verified remotes gossiped/fetched across shards — assembled into a `WaveCertificate`. Since every EC binds root to outcomes (recompute-on-decode, INV-EXEC-2) and the roots must match, shards cannot finalize divergent outcomes: atomic commitment is enforced by certificate content (INV-EXEC-1). The `FinalizedWave` (certificate + attested local receipts) then rides in a subsequent block, locks release, and the transaction is terminal.
...
- **Conflict detection** (`ConflictDetector`). A true cross-shard deadlock requires bidirectional overlap: a remote transaction's source entries overlap what a local transaction needs from that shard *and* the remote's targets overlap what the local one owns locally. Detected on provision commit (forward) and on local registration (reverse), resolved by deterministic tiebreak — lower transaction hash wins, the loser aborts before executing. Both shards derive identical conflicts from identical committed inputs (INV-EXEC-4).
...
- **INV-EXEC-1 (atomicity).** A cross-shard transaction reaches the same terminal outcome on every participating shard; finalization requires matching-root ECs from all of them.
- **INV-EXEC-2 (certificate soundness).** An EC's receipt root always matches its outcome vector (recomputed at decode); quorum and signature are verified against the WT-resolved committee.
- **INV-EXEC-3 (partial coupling).** No two in-flight-or-ready transactions share a declared node; locks persist until wave finalization.
- **INV-EXEC-4 (deterministic conflicts).** Cross-shard conflict verdicts derive from committed state only, with a total-order tiebreak; all replicas and both shards agree.
- **INV-EXEC-5 (termination).** Every wave reaches a terminal outcome by its attested deadline; unprovisioned waves all-abort.
- **INV-EXEC-6 (ownership determinism).** Ownership merge is a deterministic rule (a vault claimed by both a local and a remote owner aborts the transaction); divergent write placement cannot occur.
- **INV-EXEC-7 (attested retention).** All DA retention and eviction decisions key on BFT-attested time; positive-ack (EC coverage) or provable expiry are the only eviction causes before the horizon.
- **INV-EXEC-8 (divergence containment).** A replica whose local execution disagrees with its shard's EC never finalizes its own result; recovery is via the canonical chain.
- **INV-EXEC-9 (declared bounds).** Writes outside the declared/derived set are dropped identically on all replicas.
- **INV-EXEC-10 (artifact trust).** Cross-shard inputs are trusted only as proofs against QC-attested remote content — never as peer messages.
