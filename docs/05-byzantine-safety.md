...
**Per committee, classical BFT.** Every consensus decision — shard QC, execution certificate, beacon agreement — is made by a committee of size n assuming at most f Byzantine members with n ≥ 3f+1. Quorums are strict >2/3 (`VoteCount::has_quorum`); voting power is per seat (one validator, one vote), with stake acting as an admission gate rather than as vote weight — a dynamic, demand-tracking minimum stake governs who may be in the pool at all ([06-resource-economics.md](06-resource-economics.md)). Byzantine members may equivocate, withhold, delay, collude, and time messages adversarially. The network is partially synchronous: safety never depends on timing; liveness resumes under eventual synchrony.
