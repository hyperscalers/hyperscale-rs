...
- **Unpredictable assignment.** Committees are sampled (`sample_committee`, pool draws) from beacon randomness — an accumulator re-mixed every epoch from the shard blocks' mandatory reveal leaves: every shard block carries a key-fixed hash-VRF output bound to its slot, and the fold consumes exactly the watermark-to-boundary range of each shard's accumulator, so no party chooses the folded set. Assignment for epoch e+1 is determined only at epoch e's fold, so an adversary cannot know placements far ahead. Interior reveals are committed before any later leaf exists, blind even to their own producers; the residual influence is confined to the window edge, where the proposer closing a shard's epoch window picks among a handful of already-known candidate folds or forfeits its slot for a blind redraw — a one-sided lever gated on holding that slot, priced in [committee_security.md](../specs/committee_security.md) §10. An epoch where no shard chunk folds (bootstrap, or suppressing every shard's crossing at once — a self-announcing attack) falls back to mixing the beacon proposals' own VRF reveals, which carries the classical last-revealer grind: a Byzantine beacon member toggles whether its key-fixed output joins the seed and enumerates `2^t` candidate seeds. The two failure thresholds are defended differently either way: reaching f+1 corrupt halts the shard — a recoverable *liveness* outcome that a beacon-detected halt undoes by re-drawing the whole committee — while reaching 2f+1 is a *terminal*, unrecoverable compromise (unilateral control mints stake and cascades) with no after-the-fact containment, so its defense is entirely *preventing the march*: the grind-resistant seed plus a small beacon committee, FIFO eviction, jail-on-first for withholding, a recency-weighted resample, and pool hygiene (β low) together hold a targeted shard below 2f+1 across the tolerated corruption band. (A uniform `id % num_shards` layout exists only as the static genesis/test bootstrap constructor.)
