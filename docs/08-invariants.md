...
| ID | Class | Property |
|---|---|---|
| **INV-BEACON-1** | Safety | **One block per epoch.** SPC/PC (plus the skip path) commit at most one beacon block per epoch; honest replicas agree on the block sequence. *(Known interim gap: the pool skip certificate is a second commit path whose quorum need not intersect the committee's, so a partition outlasting the skip deadline can leave both an SPC block and a skip block validly certified for one epoch. Mitigated, not closed: honest validators sign at most one of the two paths per epoch, and a replica that verifies a competing block for its adopted epoch halts with the evidence rather than continuing on a forked chain — see [01 §3.1](01-consensus-layers.md).)* |
...
| **INV-BEACON-7** | Liveness | **Epoch progress.** A stalled view is skipped by an f+1 indirect certificate; an unrecoverable committee is skipped by a 2f+1 pool skip certificate; epoch production is paced to wall-clock, never ahead of it. |
...
| **INV-RESHAPE-7** | Safety | **Staffing stability.** Shuffles skip mid-reshape shards; merge keepers are pinned; cohort draws are frozen-seed re-derivable, so a lapse/re-assert cycle cannot move an observer's sync target. |
...
- **Known deliberate gaps** (verification should treat these as documented caveats, not discoveries): safe-vote registers are not yet persisted across restarts (conservative re-init from `high_qc`; crash-equivocation window acknowledged — [01 §1.2](01-consensus-layers.md)); the beacon's dual commit path (INV-BEACON-1 note — detected and halted, not reconciled); provision `owned_nodes` attestation granularity (INV-EXEC-10 note); INV-SEC-1 is an assumption whose statistical parameters (committee size vs. pool corruption) deserve their own analysis; committee selection is identity-only, so co-hosted vnodes share a fault domain — host-spread is deferred to deployment tooling ([06 §4](06-resource-economics.md)).
- **Existing mechanized anchors.** The `fork_safety` adversarial harness (asserts no committable fork), the determinism replay test (asserts INV-DET-4), and the straddler-atomicity scenarios (assert INV-RESHAPE-5 end to end on both harnesses) are the executable counterparts to start cross-validating models against.
