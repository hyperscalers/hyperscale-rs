...
- **Known deliberate gaps** (verification should treat these as documented caveats, not discoveries): safe-vote registers are not yet persisted across restarts (conservative re-init from `high_qc`; crash-equivocation window acknowledged — [01 §1.2](01-consensus-layers.md)); the beacon's ratification vote registers and the process-level signing fence share the same posture (a crashed-and-restarted pool member can double-vote a round; at the tightest pool sizes a single such crash plus f Byzantine meets the dual-commit bound — the same window as the shard registers, so persistence is one deferred mechanism for both layers); provision `owned_nodes` attestation granularity (INV-EXEC-10 note); INV-SEC-1 is an assumption whose statistical parameters (committee size vs. pool corruption) deserve their own analysis; committee selection is identity-only, so co-hosted vnodes share a fault domain — host-spread is deferred to deployment tooling ([06 §4](06-resource-economics.md)).
