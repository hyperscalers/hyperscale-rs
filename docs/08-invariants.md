...
| **INV-RESHAPE-5** | Safety | **Straddler fence.** A wave naming a past-terminal shard P finalizes anywhere iff `wave_id ∈ S_P` (P's on-chain settled set at its terminal block). Unknown set ⇒ defer; absent ⇒ reject; past horizon ⇒ reject. Preserves INV-EXEC-1 across shard termination. |
