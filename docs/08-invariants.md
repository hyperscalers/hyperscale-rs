...
| **INV-SEC-3** | Safety | **Accountable expulsion.** Beacon-ballot (PC vote) equivocation evidence is self-proving and permanently revokes the key (removal, not slashing — stake is preserved and withdrawable). Shard-vote equivocation is likewise self-proving: prevented for honest keys by the vote discipline (INV-SHARD-2/3), detected on receipt, and — when a replica holds both votes — carried to the beacon on a QC-attested `VoteEquivocation` witness leaf that permanently revokes the key. Liveness jailing derives only from merkle-proven, exactly-once-applied witness leaves and lifts after a cooldown. Accountability is structural: a jailed seat is single-use and re-seats only through the beacon's random draw, never by operator choice. |
