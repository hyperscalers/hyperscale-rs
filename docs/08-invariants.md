...
**Suggested verification order** (dependency-first): INV-SEC-1 (the premise) → INV-SHARD-1..9 (single-chain safety) → INV-BEACON-1..8 (topology determinism) → INV-EXEC-1..10 (atomic commitment) → INV-RESHAPE-1..8 (atomicity under topology change) → INV-SEC-2..14 / INV-STATE / INV-ECON / INV-DET (the supporting mechanisms and reductions).
