...
For a cross-shard wave, each participating shard produces its own EC over the same outcome set. The **wave certificate** bundles them — exactly one local EC plus a verified EC from every remote participant — and only that bundle finalizes the wave. Agreement across shards is enforced by content, not coordination: all ECs must carry the same receipt root, and since execution is deterministic, honest quorums on every shard compute the same one. A `FinalizedWave` (certificate plus the local receipts it attests) rides in a subsequent block, making execution results part of the ordered chain; receipts are validated against the EC's attestation before they are accepted from any source.
...
1. **Lookahead (L=1).** Epoch `e+1`'s committees are computed and frozen during epoch `e`'s fold — before any artifact stamped into epoch `e+1`'s window can exist (INV-BEACON-3). No verifier ever needs a committee that isn't already determined. The freeze also pre-positions validators: a member joining a committee next epoch has the current epoch to sync.
