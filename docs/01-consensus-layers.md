...
**SPC (strong prefix consensus)** sequences PC instances across views within an epoch. A view is entered via a signed proposal object from that view's leader; if the leader stalls, f+1 members exchange empty-view reports of their highest certified triple, aggregating into an *indirect certificate* that skips forward while pinning the next leader to a specific predecessor value — compressing view synchronization into one hash-sized commitment rather than a value re-broadcast. A separate **skip path** lets 2f+1 of the wider pool abandon an epoch whose committee is unrecoverable, so a bad committee draw cannot wedge the chain (INV-BEACON-7). Full-coverage feeding with a bounded dwell ensures PC inputs are complete views rather than racy partial ones (an eager-feed variant demonstrably collapsed prefixes to empty under load).

The two commit paths' quorums need not intersect: a partition that outlasts the skip deadline can leave both a proposal-carrying SPC block and a skip block validly certified for one epoch — a known interim gap in INV-BEACON-1, mitigated rather than closed. Honest validators sign at most one of the two paths per epoch (a round-3 PC vote excludes a skip request and vice versa, which forces quorum overlap whenever the pool is not much larger than the committee), a proposal-less SPC block is bit-identical to the skip block and converges benignly, and a replica that verifies a competing block for an epoch it has already adopted halts with the evidence rather than extending a forked chain.

An epoch's agreement yields a `BeaconBlock`: the committed proposal set plus per-shard contributions, authenticated by a `BeaconCert` (SPC certificate, skip certificate, or the genesis config hash).
