...
**`t_admit` — the clearing price for exactly `target` seats.** Every stake pool implicitly *offers* seats at descending prices: a pool with effective stake `S` can support 1 validator at price `S`, 2 at `S/2`, 3 at `S/3`, and so on. The fold gathers all offerings from all pools, sorts them descending, and takes the `target`-th one — the marginal price at which the pools would collectively supply exactly the target population. This is where the self-regulation lives:
...
- **Auto-reactivation.** Each epoch, a sweep promotes `InsufficientStake` validators back to `Pooled` wherever capacity has reappeared — and because each promotion changes an active count, which can change `t_no_eject`, which can change the global price, the sweep refreshes `min_stake` after every flip and runs to a fixpoint. Recovery is automatic; there is no manual re-activation transaction.
...
- **INV-ECON-1 (no ejection by repricing).** `min_stake` never exceeds the tightest pool's per-validator budget (`t_no_eject`); price movement alone never deactivates a sitting validator — only stake withdrawal can.
- **INV-ECON-2 (sybil floor).** `min_stake ≥ MIN_STAKE_FLOOR` always; a seat never becomes free, however short the network is.
- **INV-ECON-3 (pool solvency).** Every pool's active validator count is always supportable at the current price: `active_count ≤ floor(effective_stake / min_stake)`, enforced at registration and unjail, restored by auto-deactivation on withdrawal maturation.
- **INV-ECON-4 (supply elasticity).** The admission price is the market-clearing marginal offering for exactly the target population (`committees × shard_size + buffer`, computed from lookahead committees); abundance raises it, scarcity lowers it toward the floor, so the pool refills exactly when demand outruns supply.
- **INV-ECON-5 (deterministic pricing).** `min_stake` and every gate it feeds are pure functions of `BeaconState`; all replicas price all transitions identically, and the auto-reactivation sweep reprices to a fixpoint.
- **INV-ECON-6 (co-hosting safety).** Co-hosted vnodes of one validator never sign concurrently (per-identity signer seat with an epoch fence); vnode resource sharing is invisible to consensus.
